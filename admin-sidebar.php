<div class="sidebar border border-right col-lg-2 p-0 bg-body-tertiary">
    <div class="offcanvas-lg offcanvas-end bg-body-tertiary" tabindex="-1" id="sidebarMenu" aria-labelledby="sidebarMenuLabel">
        <div class="offcanvas-header">
            <h5 class="offcanvas-title" id="sidebarMenuLabel">BellBird</h5>
            <button type="button" class="btn-close" data-bs-dismiss="offcanvas" data-bs-target="#sidebarMenu" aria-label="Close"></button>
        </div>
        <div class="offcanvas-body d-md-flex flex-column p-0 pt-lg-3 overflow-y-auto">
            <ul class="nav flex-column">
                <li class="nav-item">
                    <a class="nav-link" aria-current="page" href="admin-dashboard.php">
                        <i class="bi bi-house-fill">&nbsp;</i>Dashboard
                    </a>
                </li>

                <li class="nav-item">
                    <a class="nav-link " href="admin-product.php">
                        <i class="bi bi-cart3">&nbsp;</i>Products
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link " href="admin-user.php">
                        <i class="bi bi-people-fill">&nbsp;</i>Customers
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link " href="admin-stock.php">
                        <i class="bi bi-archive-fill">&nbsp;</i>Stock
                    </a>
                </li>
            </ul>

            <h6 class="sidebar-heading d-flex justify-content-between align-items-center px-3 mt-4 mb-1 text-body-secondary text-uppercase">
                <span>Reports</span>
                <a class="link-secondary" href="#" aria-label="Add a new report">

                </a>
            </h6>
            <ul class="nav flex-column mb-auto">
                <li class="nav-item">
                    <a class="nav-link " href="user-report.php">
                        <i class="bi bi-card-text"></i>
                        User Reports
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link " href="product-report.php">
                        <i class="bi bi-card-text"></i>
                        Product Reports
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link " href="stock-report.php">
                        <i class="bi bi-card-text"></i>
                        Stock Reports
                    </a>
                </li>

            </ul>

            <hr class="my-3">

            <ul class="nav flex-column mb-auto">
                <li class="nav-item">
                    <a class="nav-link " href="#">

                       Feedbacks
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link " href="admin-signin.php">
                        <i class="bi bi-door-closed"></i>
                        Sign out
                    </a>
                </li>
            </ul>
        </div>
    </div>
</div>